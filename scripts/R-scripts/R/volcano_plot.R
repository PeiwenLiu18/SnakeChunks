...
VolcanoPlot <- function(
  ...
  signif.alpha <- multitest.table[,control.type] <= alpha
  message("\t\t", sum(signif.alpha), " features significant with alpha threshold (", alpha, ") on ", control.type)
  # table(positive)
  positive <- signif.alpha 
  
  ## Apply threshold on effect size if required
  if (!is.null(effect.threshold)) {
    sufficient.effect <- abs(multitest.table[,effect.size.col]) >= effect.threshold
    message("\t\t", sum(sufficient.effect), " features with effect size (", effect.size.col,") > ", effect.threshold)
    positive <- positive & sufficient.effect
  }
  message(sum(positive), " features passing the thresholds")

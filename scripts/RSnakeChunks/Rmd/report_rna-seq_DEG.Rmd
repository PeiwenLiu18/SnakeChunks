...
if (exists("gtf.file")) {
  ...
  message.verbose(paste("Loading gene annotations from GTF file: ", gtf.file), 2)
...
message.verbose(paste("Reading sample description file: ", sample.description.file), 2)
...
par(mfrow=c(2,2))
  ...
  BoxplotsPerSample(all.counts, stats.per.sample, xlab="Raw counts", 
                main="Raw counts per sample")

  ## Boxplot of log10-transformed counts
  BoxplotsPerSample(all.counts.log10, stats.per.sample, xlab="log10(counts)", 
                main="log10(counts) per sample")

  ## Boxplot of CPMs
  BoxplotsPerSample(cpms, stats.per.sample, xlab="CPM", 
                main="CPMs per sample")
  
  ## Boxplot of log10-transformed CPMs
  BoxplotsPerSample(cpms.log10, stats.per.sample, xlab="log10(CPM)", 
...
################################################################
message.verbose("Plotting betwen-replicate comparisons", 2)
...
#for (cond in conditions.with.replicate) {
  message.verbose(paste(sep="", "\tcondition\t", cond), 3)
...
message.verbose(paste("Reading design file: ", design.file), 2)
...
# View(result.table)
  ...
  message.verbose("Exporting tables with up-regulated genes only")
  ...
  message.verbose(paste(sep="", "\t\tSelected DEG file with up-regulated genes\t", deg.file.up), 1)

  message.verbose("Exporting tables with down-regulated genes only")
  ...
  message.verbose(paste(sep="", "\t\tSelected DEG file with down-regulated genes\t", deg.file.down), 1)

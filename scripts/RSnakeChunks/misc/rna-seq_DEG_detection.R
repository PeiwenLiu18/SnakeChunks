...
message.verbose(paste("Reading parameters", r.params.path), 1)
...
message.verbose("Reading sample descriptions", 1)
...
message.verbose("Reading design", 1)
...
message.verbose("Loading count table", 1)
...
message.verbose("Computing count-derived metrics (log-transformed)", 1)
...
message.verbose(paste("Treating zero-values by adding epsilon =", epsilon), 2)
...
if (exists("gtf.file")) {
  ...
  message.verbose(paste("Loading gene information from GTF file", gtf.file))
  ...
  message.verbose(paste("No GTF file has been specified"))
  g <- nrow(all.counts)
...
if (exists("gene.info.file")) {
  message.verbose(paste("Loading gene information from tab-delimited file", gene.info.file))
...
message.verbose(paste("Exporting gene information table", gene.info.out), 1)
write.table(x = gene.info, row.names = FALSE,
            file = gene.info.out, sep = "\t", quote=FALSE)
message.verbose(paste(sep="", "\tGene info table\t", gene.info.out), 1)


################################################################
## Compute sample-wise statistics on mapped counts
################################################################
message.verbose("Computing statistics per sample", 1)
...
################################################################
message.verbose("Computing CPMs", 1)
...
message.verbose("Exporting stats per sample", 1)
...
message.verbose(paste(sep="", "\tSummary per sample\t", sample.summary.file), 1)
...
################################################################
message.verbose("Plotting betwen-replicate comparisons", 1)
...
#for (cond in conditions.with.replicate) {
  message.verbose(paste(sep="", "\tcondition\t", cond), 2)
...
################################################################
message.verbose("Starting differential analysis", 1)
...
for (i in 1:nrow(design)) {
  ...
  message.verbose(paste(sep="", "\tDifferential analysis\t", i , "/", nrow(design), "\t", cond1, " vs ", cond2), 1)
  ...
  message.verbose(paste(sep="", "\t\tDESeq2 result file\t", deseq2.result.file), 1)

  ################################################################
  ## Export DESeq2 plots
  message.verbose("\t\tExporting DESeq2 plots", 2)
  ...
  ## MA plot
  message.verbose("\t\t\tDESeq2 MA plot", 2)
  ...
  message.verbose("\t\t\tDESeq2 PCA plot", 2)
  ...
  ## edgeR analysis
  ################################################################
  message.verbose("\t\tedgeR analysis", 2)
  ...
  message.verbose(paste(sep="", "\t\tedgeR result file\t", edger.result.file), 1)

  ################################################################
  ## Export edgeR plots

  message.verbose("\t\tGenerating edgeR plots", 2)

  ## Smear plot
  message.verbose("\t\t\tedgeR Smear plot", 3)
  ...
  message.verbose("\t\t\tedgeR MDS plot", 3)
  ...
  message.verbose("\t\t\tedgeR mean-variance relationship", 3)
  ...
  message.verbose("\t\t\tedgeR BCV plot", 3)
  ...
  message.verbose("\t\tComparing DESeq2 and edgeR results", 2)
  ...
  message.verbose(paste(sep="", "\t\tMerged result file\t", result.file), 1)


  ################################################################
  ## Plots to compare edgeR and DESeq2 results
  message.verbose("\t\tComparison plots between edgeR and DESeq2", 2)
  ...
  message.verbose("\t\tmean CPM plot", 2)
...
#       message.verbose(paste(sep=" ", "\t\t", deg.tool, criterion, "Volcano plot"), 3)
...
#    }
  ...
  # names(result.table)
  message.verbose("Storing differentially expressed genes in a two-column file (gene-cluster)", 1)
  ...
  message.verbose(paste(sep="", "\t\tCluster file\t", cluster.file), 1)
  ...
  if (run.functional.enrichment & exists("org.db") & !is.na(org.db) & !is.null(org.db) & exists("gene.info.rsat")) {
    ...
    message.verbose("Starting the analysis of functional enrichment")

    ## Convert IDs to entrez IDs
    message.verbose("Converting gene names to Entrez IDs")
    ...
    for (col in geneset.selection.columns) {
      message.verbose(paste("Gene selection column", col))
      ...
      message.verbose(paste("\t", gene.nb, "selected genes"))
      if (gene.nb == 0) {
        message.verbose(paste("\t", "Skipping: not a single gene selected"))
        next
      }

      # length(geneset)
      message.verbose(paste("\tGeneFunctional enrichment"))
      ...
      message.verbose(paste(sep="", "\tGOstats BP over-representation\t",
      ...
      if ((exists("go.map.file")) & (exists("go.description.file"))) {
        ...
        message.verbose(paste(sep="", "\tGO clusterProfiles::enricher\t",
...
message.verbose("Exporting summary table", 1)
...
message.verbose(paste(sep="", "\tSummary per analysis\t", summary.file), 1)
...
if (run.param$save.image) {
  ...
  message.verbose(paste("Saving memory image", image.file), 1)

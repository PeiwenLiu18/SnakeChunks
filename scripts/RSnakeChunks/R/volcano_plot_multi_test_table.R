...
#' @param xlab=paste(sep="","Effect_size_(",effect.size.col,")") Label for the X axis.
...
#'
#' @export
VolcanoPlot.MultiTestTable <- function(
  multitest.table,
  effect.size.col,
  ...
  plot.points = TRUE,
  plot.ci = FALSE,
  xlab = paste(sep = "","Effect size (", effect.size.col, ")"),
  ...
  positive <- !is.na(multitest.table[,control.type] ) & (multitest.table[,control.type] <= alpha)
  # summary(positive)
  ...
  if (density.colors) {
    ...
    if (length(col.points) != nrow(multitest.table.sorted)) {
      if (is.na(col.positive) | is.null(col.positive)) {
        stop("WHAT IS THIS")
        multitest.table.sorted[positive, "color"] <- rep(NA, times=sum(positive)) ## 2018-07-26: why do I set colors to NA ?
      } else {
        multitest.table.sorted[positive, "color"] <- rep(col.positive, length.out = sum(positive, na.rm = TRUE))
      }
    }
  }

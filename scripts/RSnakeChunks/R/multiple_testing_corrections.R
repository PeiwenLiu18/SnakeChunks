...
#' PlotPvalDistrib.MultiTestTable(multitest.result)
#'
...
#                                       file.prefix=NULL,
  ...
  multitest.table$rank <- rank(p.values, na.last = TRUE)
  ...
  #   par(mfrow=c(1,1))
  # hist(multitest.table$p.value, breaks=100)
  # hist(multitest.table$qval.0, breaks=100)

  ## Estimate numbers of null and alternative hypotheses
  m0.est <- min(m, sum(!is.na(p.values) & p.values >= lambda) / (1 - lambda)) ## m0 cannot be larger than m
  m1.est <- m - m0.est
  pi0 <- m0.est/m
